...
void sparsify (double *M,int32_t M_num,int32_t dim,int32_t* idx_sparse,int32_t &idx_size,double min_dist,int32_t idx_start) {
...
void mexFunction (int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[]) {
  ...
  // check arguments
  if (nrhs!=3)
    mexErrMsgTxt("3 input parameters required (M,min_dist,start_idx).");
  if (nlhs!=1)
  ...
  int32_t  idx_start  = (int32_t)*((double*)mxGetPr(prhs[1]));

  int32_t *idx_sparse = (int32_t*)calloc(M_num,sizeof(int32_t));
  int32_t  idx_size   = 0;

  // sparsify
  sparsify(M,M_num,dim,idx_sparse,idx_size,min_dist,idx_start);
